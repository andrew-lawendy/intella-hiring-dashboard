...
export function CandidateCard({
  ...
  return (
    ...
    >
      {banner && (
        <div
          className={`px-4 py-1.5 text-[12px] font-semibold uppercase tracking-[0.08em] flex items-center gap-1.5 ${banner.cls}`}
        >
          {banner.label}
        </div>
      )}
        ...
        <span className="flex items-center gap-2 flex-wrap text-[11.5px] font-medium text-muted-foreground">
          ...
          {verdictInfo && (
            <>
              <span className="w-1 h-1 rounded-full bg-border flex-shrink-0" aria-hidden="true" />
              <span
                className="text-[12px] font-semibold uppercase tracking-[0.05em] px-2 py-0.5 rounded-full"
                style={{
